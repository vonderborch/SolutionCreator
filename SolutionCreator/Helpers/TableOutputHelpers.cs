...
{
    public static class TableOutputHelpers
    {
        /// <summary>
        ...
        public static string ConvertToTable(List<string> columns, List<List<string>> rows, int maxColumnSize = int.MaxValue, bool addSeparatorText = true, bool includeHeaders = true)
        {
            ...
            {
                throw new Exception("There must be the same number of columns in each row, and headers for those columns!");
            }
        ...
        private static string GetRowText(List<string> text, List<int> maxColumnSize, char padCharacter = ' ')
        {
            ...
            for (var i = 0; i < maxNeededLines; i++)
            {
                ...
                var txt = rowText[0].Count > i ? rowText[0][i] : string.Empty;
                linePart.Append($"{txt.PadRight(maxColumnSize[0], padCharacter)}");
        ...
        private static List<List<string>> GetRowSplitListText(List<string> row, List<int> maxColumnSize)
        {
            ...
            {
                output.Add(SplitStringByChunkSize(row[i], maxColumnSize[i]));
            }
